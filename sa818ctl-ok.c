...
void print_help(const char *progname)
{
    ...
    printf("  -r, --rssi <interval>" _(" Poll RSSI every <interval> seconds in monitor mode\n",
    ...
    printf("  %s --freq 145.500 --sql 3\n", progname);
    printf("  %s --port /dev/ttyUSB0 --monitor\n", progname);
    printf("\n73 de YO3ITI\n\n");
}
...
int main(int argc, char *argv[])
{
    ...
    static struct option long_opts[] = {
        ...
        {"parity", required_argument, 0, 't'},
        {"stop", required_argument, 0, 's'},
        {"bits", required_argument, 0, 'l'},
        {"sql", required_argument, 0, 'q'},
        {"freq", required_argument, 0, 'f'},
        {"narrow", no_argument, 0, 'n'},
        {"volume", required_argument, 0, 'u'},
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "p:d:b:t:s:l:q:f:nu:mir:h:v:rx:tx", long_opts, NULL)) != -1)
    {
        switch (opt)
        {
        case 'p':
